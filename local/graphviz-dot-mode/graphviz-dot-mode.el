...
(defcustom graphviz-dot-use-temp-files t
  "*Should buffers that are not attached to files be saved as
  temporary files in order to compile and view them?"
  :type 'boolean
  :group 'graphviz)
...
(defun graphviz-set-compile-command (f-name)
  (when f-name
...
`graphviz-dot-mode-hook' with no args, if that value is non-nil."
  ...
  (when (buffer-file-name)
    (setq-local compile-command
                (graphviz-set-compile-command (buffer-file-name))))
